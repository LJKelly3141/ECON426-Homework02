...
- Load the **cps5** dataset from the local data folder.
...
**Explanation:**
This code checks if the **pacman** package is installed and installs it if necessary. It then loads the tidyverse collection for data manipulation and visualization. Next, it loads the cps5 dataset from the local `data/` folder. Finally, the `glimpse()` function provides a quick overview of the dataset's structure (variables and their types) so you know what data you're working with.
